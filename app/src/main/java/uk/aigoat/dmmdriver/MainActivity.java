...
public class MainActivity extends Activity {
                ...
                if (webView != null) {
                    webView.evaluateJavascript("(!!document.getElementById('driverPortal')).toString()", value -> {
                        ...
                        if (pendingButton != null) {
                            pendingButton.setVisibility(driverVisible ? View.VISIBLE : View.GONE);
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        s.setUserAgentString(s.getUserAgentString() + " DMM-Android-Driver/3.12 SQLiteOffline/3 AtomicCompletion/3 NativePendingUI/1 CameraChooser/1");
    ...
    private void installMinimalJobPersistence(WebView view) {
        String js = "(function(){try{" +
            "if(window.__DMM312)return;window.__DMM312=true;" +
            "if(!window.DMMNative)return;" +
            "var KEY='dmmJobsV3';" +
            "var originalSet=Storage.prototype.setItem,originalGet=Storage.prototype.getItem,originalRemove=Storage.prototype.removeItem;" +
            "Storage.prototype.setItem=function(k,v){" +
                "if(this===localStorage&&k===KEY){" +
                    "try{var a=JSON.parse(String(v)||'[]');if(Array.isArray(a)){for(var i=0;i<a.length;i++){var j=a[i];if(j&&j._pendingCloudUpload===true&&String(j.status||'').toLowerCase()==='delivered'){var r=JSON.parse(DMMNative.completeJobAtomic(JSON.stringify(j)));if(!r.ok)throw new Error(r.error||'SQLite completion verification failed');}}}" +
                    "if(!DMMNative.setJobsJson(String(v)))throw new Error('SQLite jobs write failed');return;}catch(e){throw e;}" +
                "}" +
        ...
        "}catch(e){console.error('DMM v3.12 persistence install failed',e);}})();";
    ...
    private void refreshPendingButton() {
        ...
        runOnUiThread(() -> {
            int n = 0;
            try { n = offlineDatabase.pendingCount(); } catch (Exception ignored) {}
        ...
        try {
            ...
            if (rows.length() == 0) {
                ...
                for (int i = 0; i < rows.length(); i++) {
                    ...
                    if (!err.isEmpty() && !"null".equalsIgnoreCase(err)) text.append("\nError: ").append(err);
    ...
    private void launchFileChooser() {
        try {
            ...
            Toast.makeText(this, "Unable to open camera/photo chooser", Toast.LENGTH_LONG).show();
        }
    }
    ...
    @Override protected void onDestroy() {
        uiHandler.removeCallbacks(pendingRefresh);
        if (webView != null) {
