...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState){
        ...
        WebSettings s=webView.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setLoadsImagesAutomatically(true);s.setUseWideViewPort(true);s.setLoadWithOverviewMode(true);s.setAllowFileAccess(true);s.setAllowContentAccess(true);s.setMediaPlaybackRequiresUserGesture(false);s.setCacheMode(isOnline()?WebSettings.LOAD_DEFAULT:WebSettings.LOAD_CACHE_ELSE_NETWORK);s.setUserAgentString(s.getUserAgentString()+" DMM-Android-Driver/3.27 ExternalIntents/1 FastSync/1 StickyPending/2 NativeCamera/3 WebCameraPermission/2 CameraPreviewAutoplay/1 AppScrollRail/1");
        CookieManager.getInstance().setAcceptCookie(true);CookieManager.getInstance().setAcceptThirdPartyCookies(webView,true);
        webView.setWebViewClient(new WebViewClient(){
            @Override public void onPageFinished(WebView view,String url){super.onPageFinished(view,url);installOfflineAndFastSync(view);installAppScrollRail(view);refreshPendingButton();}
    ...
    private void installAppScrollRail(WebView view){
        String js="(function(){try{"+
            "if(window.__DMM_APP_SCROLL_RAIL)return;window.__DMM_APP_SCROLL_RAIL=true;"+
            "var rail=document.createElement('div');rail.id='dmmAppScrollRail';rail.setAttribute('aria-label','Page scroll controls');"+
            "rail.innerHTML='<button type=\"button\" class=\"dmm-scroll-up\" aria-label=\"Scroll up\">&#9650;</button><div class=\"dmm-scroll-track\"><div class=\"dmm-scroll-thumb\"></div></div><button type=\"button\" class=\"dmm-scroll-down\" aria-label=\"Scroll down\">&#9660;</button>';"+
            "var style=document.createElement('style');style.textContent='#dmmAppScrollRail{position:fixed;z-index:2147483000;right:5px;top:54px;bottom:8px;width:40px;display:flex;flex-direction:column;gap:5px;padding:5px;background:rgba(229,231,235,.96);border:1px solid #9ca3af;border-radius:12px;box-shadow:0 2px 10px rgba(0,0,0,.22);box-sizing:border-box;touch-action:none}#dmmAppScrollRail button{width:30px;height:42px;min-height:42px;margin:0;padding:0;border:0;border-radius:8px;background:#1f2937;color:#fff;font-size:18px;font-weight:900;line-height:42px;text-align:center}#dmmAppScrollRail button:active{background:#2563eb}.dmm-scroll-track{position:relative;flex:1;min-height:70px;background:#cbd5e1;border-radius:9px;overflow:hidden}.dmm-scroll-thumb{position:absolute;left:4px;right:4px;top:0;min-height:54px;background:#374151;border:2px solid #f8fafc;border-radius:9px;box-sizing:border-box}';document.head.appendChild(style);document.body.appendChild(rail);"+
            "var track=rail.querySelector('.dmm-scroll-track'),thumb=rail.querySelector('.dmm-scroll-thumb'),drag=false,startY=0,startTop=0;"+
            "function maxScroll(){return Math.max(0,document.documentElement.scrollHeight-window.innerHeight);}"+
            "function update(){var max=maxScroll();rail.style.display=max>4?'flex':'none';if(!max)return;var th=Math.max(54,Math.round(track.clientHeight*(window.innerHeight/document.documentElement.scrollHeight)));th=Math.min(track.clientHeight,th);thumb.style.height=th+'px';thumb.style.top=Math.round((window.scrollY/max)*(track.clientHeight-th))+'px';}"+
            "function page(dir){window.scrollBy({top:dir*Math.max(240,Math.round(window.innerHeight*.72)),left:0,behavior:'smooth'});}"+
            "rail.querySelector('.dmm-scroll-up').addEventListener('click',function(e){e.preventDefault();e.stopPropagation();page(-1);});rail.querySelector('.dmm-scroll-down').addEventListener('click',function(e){e.preventDefault();e.stopPropagation();page(1);});"+
            "thumb.addEventListener('pointerdown',function(e){drag=true;startY=e.clientY;startTop=parseFloat(thumb.style.top)||0;thumb.setPointerCapture(e.pointerId);e.preventDefault();e.stopPropagation();});"+
            "thumb.addEventListener('pointermove',function(e){if(!drag)return;var limit=Math.max(1,track.clientHeight-thumb.offsetHeight),top=Math.max(0,Math.min(limit,startTop+e.clientY-startY));window.scrollTo(0,(top/limit)*maxScroll());e.preventDefault();});"+
            "thumb.addEventListener('pointerup',function(e){drag=false;e.preventDefault();});thumb.addEventListener('pointercancel',function(){drag=false;});"+
            "track.addEventListener('pointerdown',function(e){if(e.target===thumb)return;var r=track.getBoundingClientRect(),ratio=Math.max(0,Math.min(1,(e.clientY-r.top)/r.height));window.scrollTo({top:ratio*maxScroll(),left:0,behavior:'smooth'});e.preventDefault();e.stopPropagation();});"+
            "window.addEventListener('scroll',update,{passive:true});window.addEventListener('resize',update);new MutationObserver(function(){requestAnimationFrame(update);}).observe(document.body,{childList:true,subtree:true});update();"+
        "}catch(e){console.error('DMM app scroll rail failed',e);}})();";view.evaluateJavascript(js,null);
    }
