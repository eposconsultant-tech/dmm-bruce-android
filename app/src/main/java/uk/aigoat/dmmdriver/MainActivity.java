...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        settings.setUserAgentString(settings.getUserAgentString() + " DMM-Android-Driver/3.08 SQLiteOffline/3 AtomicCompletion/2 PortalAdapter/1 KeyboardFix/2");
        ...
        webView.setWebViewClient(new WebViewClient() {
            private boolean external(String url) {
                ...
                try {
                    ...
                    if ("tel".equals(s)) { startActivity(new Intent(Intent.ACTION_DIAL, u)); return true; }
                    if ("geo".equals(s) || "google.navigation".equals(s)) { startActivity(new Intent(Intent.ACTION_VIEW, u)); return true; }
                    if ("intent".equals(s)) { startActivity(Intent.parseUri(url, Intent.URI_INTENT_SCHEME)); return true; }
                    if (("http".equals(s) || "https".equals(s)) && ((h.contains("google.com") && url.toLowerCase().contains("maps")) || h.contains("maps.google") || h.contains("waze.com"))) {
            ...
            @Override public void onPageFinished(WebView v, String u) {
                ...
                PortalAdapter.install(v);
                v.evaluateJavascript(
                    "(function(){document.addEventListener('click',function(e){var t=e.target;if(t&&(t.tagName==='INPUT'||t.tagName==='TEXTAREA')){try{t.focus();}catch(x){}}},true);})();",
                    null
                );
            }
        });
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            @Override public void onPermissionRequest(final PermissionRequest r) {
                runOnUiThread(() -> {
                    boolean cam = false;
                    for (String x : r.getResources()) if (PermissionRequest.RESOURCE_VIDEO_CAPTURE.equals(x)) { cam = true; break; }
                    if (!cam) { r.deny(); return; }
                    ...
                    else { pendingCameraPermissionRequest = r; requestPermissions(new String[]{Manifest.permission.CAMERA}, CAMERA_PERMISSION_REQUEST); }
                });
            }
        });
    ...
    @Override protected void onActivityResult(int c, int r, Intent d) {
        if (c == FILE_CHOOSER_REQUEST) {
            if (filePathCallback != null) {
                filePathCallback.onReceiveValue(WebChromeClient.FileChooserParams.parseResult(r, d));
    ...
    @Override public void onRequestPermissionsResult(int c, String[] p, int[] g) {
        ...
        if (c == CAMERA_PERMISSION_REQUEST && pendingCameraPermissionRequest != null) {
            if (g.length > 0 && g[0] == PackageManager.PERMISSION_GRANTED) pendingCameraPermissionRequest.grant(new String[]{PermissionRequest.RESOURCE_VIDEO_CAPTURE});
    ...
    @Override protected void onSaveInstanceState(Bundle o) {
        if (webView != null) webView.saveState(o);
